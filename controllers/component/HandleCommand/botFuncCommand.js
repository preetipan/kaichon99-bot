...
async function handleUpdateAdminRole(event, actionType) {
  try {
    ...
    try {
      const member = await client.getGroupMemberProfile(
  ...
  try {
    ...
    if (closeMainMessage) {
      ...
      const userUpdatePromises = uniqueUsersToUpdate.map((userId) =>
        axios
          .patch(`${process.env.API_URL}/user/${userId}/remainingFund`)
          .then((response) => {
...
async function handleSetOdds(event, { type, odds, maxAmount }) {
  try {
    ...
    const fixedOddsText = {
      ...
      ส10: {
        x: `เสมอ แดง แทง 10 ได้ 10`,
        y: `เสมอ น้ำเงิน แทง 10 ได้ 10`,
      },
    };
    ...
    const pic = ["ตร", "สด", "ส10", "ด"].includes(type) ? "red" : "blue";
...
function calculateOdds(type, oddsValue) {
  ...
  const oddsMap = {
    ...
    1.5: { ต่อ: 1.5, รอง: 3.5 },
    1: { ต่อ: 1, รอง: 3 },
    15: { ต่อ: 15 / 1, รอง: 10 / 1.5 },
    20: { ต่อ: 1 / 20, รอง: 10 / 1 },
  };

  // ตรวจสอบว่า oddsValue มีใน oddsMap หรือไม่
  const odds = oddsMap[oddsValue];
  if (!odds) return { error: "ข้อมูลไม่ถูกต้อง" };
  ...
  if (oddsValue === 15) {
    return {
      x: `ต่อ ${side} แทง 15 ได้ 1`,
      y: `รอง ${side === "แดง" ? "น้ำเงิน" : "แดง"} แทง 10 เสีย 1.5`,
    };
  }
  if (oddsValue === 20) {
    return {
      x: `ต่อ ${side} แทง 20 ได้ 1`,
      y: `รอง ${side === "แดง" ? "น้ำเงิน" : "แดง"} แทง 10 เสีย 1`,
    };
  }
